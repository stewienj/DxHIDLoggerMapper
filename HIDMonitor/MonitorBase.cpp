...
MonitorBase::MonitorBase(HIDMonitorDLL::DeviceType deviceType, GUID deviceGuid, HIDMonitorDLL::HIDMonitorInterface^ loggerInterface) {
...
void MonitorBase::SetDevice(LPDIRECTINPUTDEVICE8 device) {
  _pDevice = device;
...
void MonitorBase::StartLoggingThread() {
  while (_keepRunning) {
    try {
      Sleep(10);
      UpdateInputState();
...
HRESULT MonitorBase::UIThreadCheckIsValid() {
  if (_deviceLost) {
...
void MonitorBase::NotifyStateChangeButton(TCHAR* controlID, LONG state, LONG previousState) {
...
void MonitorBase::NotifyStateChangeAxis(TCHAR* controlID, LONG state, LONG previousState) {
  if (state != previousState && state != -1) {
    _loggerInterface->OnHIDStateChanged(_deviceName, _deviceType, _deviceGuid, controlID, state, previousState);
  }
}
...
//-----------------------------------------------------------------------------
HRESULT MonitorBase::GetInputState(DWORD size, void* newState) {
  HRESULT hr;
